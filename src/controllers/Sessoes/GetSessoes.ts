...
export const GetSessoes = async (req, res) => {
  try {
    ...
    const responseMatter = await axios.get(
      `${url}/api/sessao/ordemdia/?o=-data_ordem&&sessao_plenaria=${id}&&page_size=30`
    );

    const arrayMatter = responseMatter.data.results;

    const mattersDefinitive = await Promise.all(
      arrayMatter.map(async (matter) => {
        const respo = await axios.get(`${url}/api/materia/materialegislativa/${matter.materia}`);
